...
def get_db_engine():
    db_url = os.getenv(
        "DATABASE_URL", "postgresql://tickets_user:tickets_pass@db:5432/tickets_db"
    )
...
with tab1:
    ...
    try:
        ...
        if df.empty:
            st.info("No tickets found in the database. Head to the Simulation tab to generate traffic.")
        else:
            # Layout for charts
            ...
            with c1:
                ...
                # Resample by 5 min
                trend = df_trend.groupby("predicted_priority").resample("5min").size().reset_index(name="count")
                
                fig_trend = px.area(
                    trend, x="timestamp", y="count", color="predicted_priority",
                    title="Incoming Tickets over Time",
                    color_discrete_map={"high": "red", "medium": "orange", "low": "green", "unknown": "gray"}
                )
...
with tab2:
    ...
    st.markdown("This report compares the latest production data against the baseline to detect concept/data drift.")
    ...
    if st.button("Fetch Latest Report"):
        if not bucket:
            ...
            try:
                ...
                st.warning(f"Could not load report from S3: {e}. Ensure the Drift Monitor job has run.")
...
with tab4:
    ...
    st.markdown("Inject mock tickets into the system to test load, graceful degradation, and data drift detection.")
    
    api_url = os.getenv("API_URL", "http://api:8000")
    
    c1, c2 = st.columns(2)
    with c1:
        num_tickets = st.number_input("Number of Tickets to Generate", min_value=1, max_value=500, value=10)
    with c2:
        drift_mode = st.checkbox("Enable Data Drift (Biased distribution towards German / Technical Support)")
    ...
    if st.button("🚀 Generate & Send"):
        with st.spinner(f"Generating {num_tickets} tickets and sending to {api_url}..."):
            ...
            if success_count > 0:
                ...
                if len(id_tuple) == 1:
                    query = f"SELECT ticket_id, subject, predicted_priority FROM tickets WHERE ticket_id = '{id_tuple[0]}'"
                else:
                    query = f"SELECT ticket_id, subject, predicted_priority FROM tickets WHERE ticket_id IN {id_tuple}"
                    
                df_results = pd.read_sql(query, engine)
                
                if not df_results.empty:
                    st.write("### Simulation Results Summary")
                    dist = df_results["predicted_priority"].fillna("unknown").value_counts().reset_index()
                    dist.columns = ["Priority", "Count"]
