...
def test_load_model_from_mlflow(mock_getenv, mock_mlflow):
    mock_getenv.side_effect = lambda key, default="": "http://fake-mlflow:5000" if key == "MLFLOW_TRACKING_URI" else default
    ...
    mock_mlflow.pyfunc.load_model.assert_called_once_with("models:/ticket-triage-classifier/Production")
...
def test_load_fallback_model(mock_getenv, mock_exists, mock_joblib, mock_mlflow):
    ...
    mock_getenv.side_effect = lambda key, default="": "" if key == "MLFLOW_TRACKING_URI" else "fake_model.joblib"
